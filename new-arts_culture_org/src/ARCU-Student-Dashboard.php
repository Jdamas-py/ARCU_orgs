...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['record_attendance'])) {
    ...
    if (empty($errors)) {
        try {
            ...
            // Redirect to avoid form resubmission
            header("Location: " . $_SERVER['PHP_SELF'] . "?attendance=success");
            exit();
        } catch (PDOException $e) {
        ...
        <div class="container-fluid">
            ...
            <div class="dropdown">
                <button class="d-flex align-items-center text-white dropdown-toggle bg-transparent border-0" id="adminDropdown" data-bs-toggle="dropdown" aria-expanded="false" type="button" style="box-shadow:none;">
                    <span class="me-2 d-none d-md-inline fw-bold">
                        <?= htmlspecialchars($student_profile['full_name'] ?? 'Student') ?>
                    </span>
                    <div class="admin-logo" aria-label="Student Panel Logo">
                        <?= strtoupper(substr($student_profile['full_name'] ?? 'S', 0, 1)) ?>
                    </div>
                </button>
                <ul class="dropdown-menu dropdown-menu-dark" aria-labelledby="adminDropdown">
                    <li class="px-3 py-2">
                        <div class="text-white">
                            <div class="fw-bold"><?= htmlspecialchars($student_profile['full_name'] ?? 'Student') ?></div>
                            <small class="text-muted"><?= htmlspecialchars($student_profile['email'] ?? '') ?></small>
                        </div>
                    </li>
                    <li><hr class="dropdown-divider" /></li>
                    <li><a class="dropdown-item" href="#" id="manageAccountLink"><i class="bi bi-gear me-2"></i>Manage Account</a></li>
                    <li><hr class="dropdown-divider" /></li>
                                        ...
                                        <table class="table table-striped table-hover" id="attendanceTable">
                                            ...
                                            <tbody>
                                                <?php if (!empty($studentAttendance)): ?>
                                                    <?php foreach ($studentAttendance as $index => $attendance): ?>
                                                        <tr>
                                                            <th scope="row">
                                                                <?= $index + 1 ?>
                                                            </th>
                                ...
                                <div class="card-body">
                                    ...
                                    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_account'])) {
                                        ...
                                        try {
                                            ...
                                            // If user_profile does not exist, insert it
                                    ...
                                    <form method="post" class="needs-validation" novalidate>
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control" id="student_id" value="<?= htmlspecialchars($student_id) ?>" readonly>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="full_name" class="form-label">Full Name</label>
                                                <input type="text" class="form-control" id="full_name" name="full_name" value="<?= htmlspecialchars($student_profile['full_name'] ?? '') ?>" required>
                                            </div>
                                        </div>
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                <label for="email" class="form-label">Email</label>
                                                <input type="email" class="form-control" id="email" name="email" value="<?= htmlspecialchars($student_profile['email'] ?? '') ?>" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="phone" class="form-label">Phone</label>
                                                <input type="tel" class="form-control" id="phone" name="phone" value="<?= htmlspecialchars($student_profile['phone'] ?? '') ?>">
                                            </div>
                                        </div>
